...
describe('Invoice Controller - Create', () => {
  ...
  describe('Successful Invoice Creation', () => {
    ...
    it('should calculate totals correctly', async () => {
      ...
      // taxTotal = 350 * 10% = 35
      expect(invoice.taxTotal).toBe(35);
      // total = 350 + 35 = 385
      expect(invoice.total).toBe(385);
    });

    it('should set paymentStatus to unpaid when total > 0', async () => {
      if (!(global as any).__MONGODB_AVAILABLE__) {
        return;
      }
      await create(mockRequest as AuthenticatedRequest, mockResponse as Response);
    ...
    it('should set paymentStatus to paid when total equals discount', async () => {
      ...
      mockRequest.body!.discount = 385; // Equal to total
    ...
    it('should generate PDF file ID', async () => {
      ...
      const jsonCall = (mockResponse.json as jest.Mock).mock.calls[0][0];
      expect(jsonCall.result.pdf).toMatch(/^invoice-.*\.pdf$/);
    });
  ...
  describe('Performance Tests', () => {
    it('should create invoice efficiently', async () => {
      if (!(global as any).__MONGODB_AVAILABLE__) {
        return;
      }
