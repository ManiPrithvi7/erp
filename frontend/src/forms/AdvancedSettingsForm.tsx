...
export default function AdvancedSettingsForm({
  isUpdateForm: _isUpdateForm = false,
}: AdvancedSettingsFormProps): JSX.Element {
  const translate = useLanguage();
    ...
    <>
      ...
      {type ? (
        type === 'select' ? (
          ...
          >
            <Input type={type as 'text' | 'number' | 'date'} />
          </Form.Item>
        )
      ) : null}
      ...
      >
        <Switch
          disabled={result ? (result as any).isCoreSetting : false}
