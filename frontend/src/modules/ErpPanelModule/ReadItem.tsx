...
import { ReadItemProps, InvoiceItem, ErpDocument, Client } from '@/types';

interface ItemProps {
  item: InvoiceItem;
  currentErp: ErpDocument;
}

const Item = ({ item, currentErp }: ItemProps): JSX.Element => {
  ...
  useEffect(() => {
    if (currentResult) {
      ...
      if (items && Array.isArray(items)) {
        ...
        setCurrentErp({ ...invoice.items, ...others, ...invoice } as ErpDocument);
      }
    }
    return () => {
      setItemsList([]);
