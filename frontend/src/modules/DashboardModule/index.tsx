...
import { SummaryData, TableColumn } from '@/types';
...
export default function DashboardModule() {
  ...
  const getStatsData = async ({ entity, currency }: { entity: string; currency: string }) => {
  ...
  const dataTableColumns: TableColumn[] = [
    ...
    },
    {
      ...
      render: (total: unknown, record: Record<string, unknown>) => {
        const currency = (record.currency as string) || undefined;
        const amount = typeof total === 'number' ? total : 0;
        return moneyFormatter({ amount, currency_code: currency });
      },
    },
    {
  ...
  const statisticCards = entityData.map((data, index) => {
    ...
    return (
      <PreviewCard
        ...
        statistics={
          !isLoading &&
          result?.performance?.map((item: { status?: string; percentage?: number }) => ({
            tag: item?.status || '',
            color: 'blue',
        ...
        <Row gutter={[32, 32]}>
          ...
          <Col className="gutter-row w-full" sm={{ span: 24 }} md={{ span: 24 }} lg={{ span: 6 }}>
            <CustomerPreviewCard
              ...
              activeCustomer={(clientResult as SummaryData)?.active}
              newCustomer={(clientResult as SummaryData)?.new}
            />
          </Col>
        </Row>
        <div className="space30"></div>
